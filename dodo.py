...
RENDERED_TEMPLATE_PATH = os.path.realpath(os.path.join(BUILD_PATH, "rendered_template"))
...
def collect_target_files(target_dir, extension):
    ...
    for file_name in sorted(os.listdir("schema-source")):
        if ".schema.yaml" in file_name:
            ...
            file_list.append(
                os.path.join(target_dir, f"{file_name_root}.schema.{extension}")
            )
    return file_list
...
def task_validate():
    ...
    return {
        "file_dep": [os.path.join("meta-schema", "meta.schema.json")]
...
def task_schema():
    ...
    return {
        "file_dep": [os.path.join("schema229", "json_translate.py")]
        + collect_source_files(),
