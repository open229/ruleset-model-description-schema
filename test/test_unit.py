...
def test_resolve_ref():
    ...
    node = schema.resolve_ref("ASHRAE229.schema.json#/definitions/Metadata")
    assert('title' not in node)

def test_get_schema_node():

    # Node in external reference
    schema = schema229.A229Schema(os.path.join(os.path.dirname(__file__),'..','build',"schema","RS0001.schema.json"))
    node = schema.get_schema_node(['description', 'product_information', 'compressor_type'])
    assert('enum' in node)

    # Node in internal reference
    schema = schema229.A229Schema(os.path.join(os.path.dirname(__file__),'..','build',"schema","RS0003.schema.json"))
    node = schema.get_schema_node(['description', 'product_information', 'impeller_type'])
    assert('enum' in node)

    # Node in nested RS
    schema = schema229.A229Schema(os.path.join(os.path.dirname(__file__),'..','build',"schema","RS0002.schema.json"))
    ...
    assert('ASHRAE 229' not in node['description'])

def test_create_grid_set():
    rep = schema229.load_json('examples/RS0004/DX-Constant-Efficiency.RS0004.a229.json')
    schema = schema229.A229Schema(os.path.join(os.path.dirname(__file__),'..','build',"schema","RS0004.schema.json"))
...
def test_process_grid_set():
    ...
    schema = schema229.A229Schema(os.path.join(os.path.dirname(__file__),'..','build',"schema","RS0004.schema.json"))
    grid_set = schema.create_grid_set(rep, ['performance','performance_map_cooling','grid_variables'])
    grid_vars2 = schema229.util.process_grid_set(grid_set)
