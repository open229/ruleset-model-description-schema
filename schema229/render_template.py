...
import schema229.markdown as markdown
...
def load_yaml_source(schema_dir, source, args_str):
    ...
    - source: string, the source key. E.g., for
      schema-source/ASHRAE229.schema.yaml, 'ASHRAE229'
...
def make_add_schema_table(schema_dir=None, error_log=None):
    ...
    def add_schema_table(
        source, table_name, caption=None, header_level_and_content=None
    ):
        ...
        if gen_table is None:
            return log_error(
                make_error_string(f'Unhandled table type "{table_name}"!', args_str),
                error_log,
            )
        return render_header(header_level_and_content) + gen_table(
            target, caption=caption, add_training_ws=False
        )
